angular.module('app')
        .controller('joblevelController', joblevelController);

joblevelController.$inject = ['$scope', '$state', '$rootScope', 'APIURL', '$http', 'ApiService']
...
function joblevelController($scope, $state, $rootScope, APIURL, $http, ApiService) {
    ...
    $scope.save = function(frm){
    	...
    	if(frm.$valid){
    		ApiService.hm_save_joblevel($scope.form_data).then(function(res){
    ...
    $scope.delete2 = function(id){
    	ApiService.hm_delete_joblevel($scope.pageInfo.actionId).then(function(res){
    ...
    $scope.change_status = function(st, id){
    	ApiService.hm_change_joblevel_status(st, id).then(function(res){
    		ApiService.notification(res.msg, 'success');
    		ApiService.hm_joblevel().then(function(res){
		    	$scope.totalItems = res.data;
		    });
