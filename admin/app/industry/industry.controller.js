...
industryController.$inject = ['$scope', '$state', '$rootScope', 'APIURL', '$http', 'ApiService']

function industryController($scope, $state, $rootScope, APIURL, $http, ApiService) {
    ...
    $scope.save = function(frm){
    	...
    	if(frm.$valid){
    		ApiService.hm_save_industry($scope.form_data).then(function(res){
    ...
    $scope.delete2 = function(id){
    	ApiService.hm_delete_industry($scope.pageInfo.actionId).then(function(res){
    ...
    $scope.change_status = function(st, id){
    	ApiService.hm_change_industry_status(st, id).then(function(res){
    		ApiService.notification(res.msg, 'success');
    		ApiService.hm_industry().then(function(res){
		    	$scope.totalItems = res.data;
		    });
